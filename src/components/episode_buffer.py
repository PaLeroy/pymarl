...
class EpisodeBatch:
    ...
    def _setup_data(self, scheme, groups, batch_size, max_seq_length,
                    preprocess):
        ...
        for s in dest.shape[::-1]:
            if v.shape[idx] != s:
                if s != 1:
                    ...
                    raise ValueError(
                        "Unsafe reshape of {} to {}".format(v.shape,
                                                            dest.shape))
            else:
                idx -= 1
    ...
    def __getitem__(self, item):
        if isinstance(item, str):
            if item in self.data.episode_data:
                ...
                [isinstance(it, str) for it in item]):
            new_data = self._new_data_sn()
            for key in item:
                if key in self.data.transition_data:
                    new_data.transition_data[key] = self.data.transition_data[
                        key]
            ...
            ret = EpisodeBatch(self.scheme, self.groups, ret_bs, ret_max_t,
                               data=new_data, device=self.device)
            return ret

    def _get_num_items(self, indexing_item, max_size):
        if isinstance(indexing_item, list) or isinstance(indexing_item,
                                                         np.ndarray):
    ...
    def _parse_slices(self, items):
        ...
        if (isinstance(items, slice)  # slice a:b
                ...
                or (isinstance(items, (
                        list, np.ndarray, th.LongTensor, th.cuda.LongTensor)))
                # [a,b,c]
        ):
...
class ReplayBuffer(EpisodeBatch):
    ...
    def insert_episode_batch(self, ep_batch):
        if self.buffer_index + ep_batch.batch_size <= self.buffer_size:
            ...
            self.episodes_in_buffer = max(self.episodes_in_buffer,
                                          self.buffer_index)
...
class ReplayBufferPopulation:
    """
    Store the transitions of each agent
    """
    ...
    def __init__(self,
                 ...
                 preprocess=None,
                 device="cpu"):
        # For each agent, create a repllay buffer
        self.buffers = {}
        self.agent_ids = []
        ...
        self.scheme = self.buffers[0].scheme

    def insert_episode_batch(self, episode_batches, agent_dict, list_matches):
    ...
    def sample(self, agent_id, agent_dict):
    ...
    def __repr__(self):
        ...
        for k, v in self.buffers.items():
            string_to_complete += "{}: ReplayBuffer of player ." \
                                  " {}/{} episodes. Keys:{} Groups:{} \n" \
                .format(k,
                        v.episodes_in_buffer,
                        v.buffer_size,
                        v.scheme.keys(),
                        v.groups.keys())
