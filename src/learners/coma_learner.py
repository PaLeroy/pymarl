...
class COMALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        q_taken = th.gather(q_vals, dim=1,
                            index=actions.reshape(-1, 1)).squeeze(1)
        pi_taken = th.gather(pi, dim=1, index=actions.reshape(-1, 1)).squeeze(
            1)
        pi_taken[mask == 0] = 1.0
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ts_logged = len(
                critic_train_stats[str(self.id_agent) + "critic_loss"])
            for key in [str(self.id_agent) + "critic_loss",
                        str(self.id_agent) + "critic_grad_norm",
                        str(self.id_agent) + "td_error_abs",
                        str(self.id_agent) + "q_taken_mean",
                        str(self.id_agent) + "target_mean"]:
                self.logger.log_stat(key,
                                     sum(critic_train_stats[key]) / ts_logged,
                                     t_env)

            self.logger.log_stat(str(self.id_agent) + "advantage_mean", (
                    advantages * mask).sum().item() / mask.sum().item(),
                                 t_env)
            self.logger.log_stat(str(self.id_agent) + "coma_loss",
                                 coma_loss.item(), t_env)
            self.logger.log_stat(str(self.id_agent) + "agent_grad_norm",
                                 grad_norm, t_env)
            self.logger.log_stat(str(self.id_agent) + "pi_max", (pi.max(dim=1)[
                                                                     0] * mask).sum().item() / mask.sum().item(),
                                 t_env)
    ...
    def _train_critic(self, batch, rewards, terminated, actions, avail_actions,
        ...
        for t in reversed(range(rewards.size(1))):
            ...
            running_log[str(self.id_agent) + "td_error_abs"].append(
                (masked_td_error.abs().sum().item() / mask_elems))
            running_log[str(self.id_agent) + "q_taken_mean"].append(
    ...
    def _update_targets(self):
        ...
        if self.id_agent == "":
            ...
            self.logger.console_logger.info(
                "Updated target network of agent_id=" + str(self.id_agent[:-1]))
