...
class ParallelRunnerMulti(ParallelRunner):
    ...
    def reset(self):
        ...
        for parent_conn in self.parent_conns:
            ...
            pre_transition_data["obs_team_1"].append(obs_team_1)
            pre_transition_data["obs_team_2"].append(obs_team_2)
    ...
    def run(self, test_mode=False):
        ...
        episode_returns = [np.zeros(self.batch.scheme['reward']['vshape']) for
                           _ in range(self.batch_size)]
        episode_lengths = [0 for _ in range(self.batch_size)]
        self.mac.init_hidden(batch_size=self.batch_size)
        terminated = [False for _ in range(self.batch_size)]
        envs_not_terminated = [b_idx for b_idx, termed in enumerate(terminated)
                               if not termed]
        ...
        while True:
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if idx in envs_not_terminated:
                    ...
                    if not terminated[idx]:
                        # Only send the actions to the env
                        # if it hasn't terminated
                        parent_conn.send(("step", cpu_actions[action_idx]))
                    action_idx += 1  # actions is not a list over every env

            # Update envs_not_terminated
            envs_not_terminated = [b_idx for b_idx, termed in
                                   enumerate(terminated) if not termed]
            ...
            self.batch.update(post_transition_data, bs=envs_not_terminated,
                              ts=self.t, mark_filled=False)
            ...
            self.batch.update(pre_transition_data, bs=envs_not_terminated,
                              ts=self.t, mark_filled=True)
        ...
        cur_stats.update({k: sum(d.get(k, 0) for d in infos) for k in
                          set.union(*[set(d) for d in infos])})
        cur_stats["n_episodes"] = self.batch_size + cur_stats.get("n_episodes",
                                                                  0)
